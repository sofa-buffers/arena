...
const MESSAGES: Record<string, { fromJSON(d: Record<string, unknown>): { serialize(os: import("@sofa-buffers/corelib").OStream): void; toJSON(): Record<string, unknown> }; decode(b: Uint8Array): { serialize(os: import("@sofa-buffers/corelib").OStream): void; toJSON(): Record<string, unknown> } }> = {
  "example": M.Example,
};
...
async function benchMain(w: string, reps: number, input: Buffer): Promise<number> {
  ...
  if (w === "encode_example" || w === "decode_example") {
    ...
    const setup = new OStream(); obj.serialize(setup);
    const wire = setup.bytes(); // setup: the decode input
    let sink = 0;
    const body = w === "encode_example"
      ? () => { const os = new OStream(); obj.serialize(os); sink ^= os.bytes().length; }
