...
//! A decoded message owns its bytes: strings, blobs and array storage all
//! come from the caller's allocator, on the one-shot path as on the streaming
//! one, so the input buffer may be reused the moment decode() returns. Pass
//! an arena and free the whole message at once.
...
pub const Example = struct {
    ...
    /// and array storage are all copied out of `data` into `alloc` (an arena
    /// frees everything at once), so `data` may be reused, overwritten or
    /// freed the moment this returns. Decoding a whole buffer costs the same
    /// copies as feeding it in pieces: the message's lifetime must not depend
    /// on which entry point produced it.
    ...
    pub const Decoder = struct {
        ...
        /// caller's either way. A stream that was REFUSED fails with that
        /// refusal's own code: the rejection is terminal, so a decoder that
        /// rejected a message cannot hand one back.
    ...
    /// arrive, instead of buffering the whole message first. A chunk is read
    /// only during the `feed` call it is passed to: reuse, overwrite or free it
    /// as soon as feed returns.
...
const _dec_Example = struct {
    ...
    ///
    /// `borrow` is always false: NEITHER path hands a chunk window to a
    /// destination. Streaming cannot -- a payload completing inside the
    /// corelib's fixed, REUSED carry buffer is indistinguishable in the callback
    /// from one in the caller's chunk, and the next stitched item overwrites it.
    /// One-shot does not either, so that a decoded message never has a shorter
    /// life than the caller thinks: the copy is what lets the input buffer be
    /// reused, overwritten or freed the moment decode() returns.
    ///
