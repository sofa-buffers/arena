...
fn fill() Example {
    return .{
        ...
        // A fixed `count` array is a FixedArray(T, N): storage plus the logical
        // length, so it takes .init(vals) rather than a bare array literal — a
        // literal would set no length and encode an empty sequence.
        .arrays = .{
            .u8 = .init(&.{ 0, 64, 128, 191, 255 }),
            .i8 = .init(&.{ -128, -64, 0, 63, 127 }),
            .u16 = .init(&.{ 0, 16384, 32768, 49151, 65535 }),
            .i16 = .init(&.{ -32768, -16384, 0, 16383, 32767 }),
            .u32 = .init(&.{ 0, 1073741824, 2147483648, 3221225471, 4294967295 }),
            .i32 = .init(&.{ -2147483648, -1073741824, 0, 1073741823, 2147483647 }),
            .u64 = .init(&.{ 0, 4611686018427387904, 9223372036854775808, 13835058055282163711, 18446744073709551615 }),
            .i64 = .init(&.{ -9223372036854775807, -4611686018427387904, 0, 4611686018427387903, 9223372036854775807 }),
            .nested = .{
                .fp32 = .init(&.{ 1.0, 2.0, 3.0, -std.math.floatMax(f32), std.math.floatMax(f32) }),
                .fp64 = .init(&.{ 1.0, 2.0, 3.0, -std.math.floatMax(f64), std.math.floatMax(f64) }),
            },
        },
        .string_array = &.{
            "Hello, Sofab!",
