...
# drops the __cxa_guard_* guard on function-local statics; -fno-use-cxa-atexit drops
# the __cxa_atexit/__dso_handle dtor-registration table for global objects (large
# .bss win for EmbeddedProto) — dead weight in firmware that never calls exit();
# -fno-unwind-tables/-fno-asynchronous-unwind-tables strip .eh_frame. This matches
# languages/cpp-embedded/footprint.sh.
