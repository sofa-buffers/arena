...
impl<'a> Visitor for V<'a> {
    ...
    fn string(&mut self, id: Id, total: usize, offset: usize, chunk: &[u8]) {
        ...
        match (self.cur, id) {
            ...
            (_Loc::Root_string_array, _) => { match sofab::seq::reserve_elem(&mut self.m.string_array, id, sofab::seq::Bound::Schema(5)) { Ok(_e) => { _e.clear(); let _ = _e.push_str(_s); if _e.len() != _s.len() { self.err = true; } } Err(_e) => { self.refuse(_e); } } }
            _ => {}
        }
    }
