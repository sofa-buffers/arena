...
mod example_dec {
    use super::*;
    use sofab::{ArrayKind, FixlenType};
...
#[allow(non_camel_case_types)] // variants spell the schema path (Root_a_b), not a type name
enum _Loc {
    Root,
    Root_nested,
    Root_arrays,
...
impl V<'_> {
    #[inline]
    fn refuse(&mut self, e: sofab::Error) {
        match e {
            sofab::Error::InvalidMsg => self.inv = true,
            _ => self.err = true,
        }
    }
}
...
impl<'a> Visitor for V<'a> {
    ...
    fn fixlen_begin(&mut self, id: Id, subtype: FixlenType, total: usize) {
        ...
        match subtype {
            FixlenType::Str => match (self.cur, id) {
                ...
                (_Loc::Root_string_array, _) => { if let Err(_e) = sofab::seq::check_index(id, sofab::seq::Bound::Schema(5)) { self.refuse(_e); return; }; if total > 64 { self.inv = true; return; }; },
                _ => {}
            },
    ...
    fn string(&mut self, id: Id, total: usize, offset: usize, chunk: &[u8]) {
        ...
        match (self.cur, id) {
            ...
            (_Loc::Root_string_array, _) => { if let Err(_e) = sofab::seq::place_elem(&mut self.m.string_array, id, sofab::seq::Bound::Schema(5), _s) { self.refuse(_e); } }
            _ => {}
        }
    }
